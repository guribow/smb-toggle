...
- 動く環境：macOS 13 以降（Apple シリコン・Intel のユニバーサル）。Intel は Mac mini 2018（macOS 15.8.1）で、アプリの起動とコマンドを確認した
- 言語：日本語・英語。Mac の言語設定が日本語なら日本語、それ以外なら英語で表示する（メニューバーのアプリもコマンドも）。アプリだけ言語を変えたいときは、システム設定 →「一般」→「言語と地域」→「アプリケーション」で指定する

## ダウンロードと入れ方

[Releases](https://github.com/guribow/smb-toggle/releases/latest) から zip をダウンロードする。日本語の説明書つきは `SMBToggle-<版>-ja.zip`、英語は `-en.zip`（アプリは同じもの）。

zip には、同じことをするコマンド `smbctl` も入っている（使い方は zip の説明書を参照）。

1. zip を開き、SMBToggle.app を「アプリケーション」フォルダへ移す
...
メニューバーにドライブのアイコンが出れば準備完了。新しい版に入れ替えたときは、また 2〜4 の許可が必要。

消し方：

1. メニューバーのアイコン →「終了」
2. SMBToggle.app をゴミ箱へ
3. データも消すときは `~/Library/Application Support/smbctl` をゴミ箱へ
4. `smbctl` を入れた場合は `sudo rm /usr/local/bin/smbctl`

注意：

- パスワードは保存しない。キーチェーンと macOS のログイン画面を使う
- プライバシー：自分で登録したサーバーにつなぐだけで、それ以外にデータを送らない。
...
| `build.sh` | ビルド（macOS 13 以降・Apple シリコンと Intel の両対応）とインストール |
| `build/` | ビルド結果（生成物） |
...
```bash
./dist.sh   # ビルドし直して dist/SMBToggle-<版>-ja.zip と -en.zip を作る
```
